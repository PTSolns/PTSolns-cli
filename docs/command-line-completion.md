`ptsolns-cli` supports command-line completion (also known as _tab completion_) for basic commands. Currently `bash`,
...
To generate the completion file you can use `ptsolns-cli completion [bash|zsh|fish|powershell] [--no-descriptions]`. By
...
Use `ptsolns-cli completion bash > ptsolns-cli.sh` to generate the completion file. At this point you can move that file
in `/etc/bash_completion.d/` (root access is required) with `sudo mv ptsolns-cli.sh /etc/bash_completion.d/`.
...
Use `ptsolns-cli completion zsh > _ptsolns-cli` to generate the completion file. At this point you can place the file in
...
1. `mv _ptsolns-cli ~/completion_zsh/`

Remember to open a new shell to test the functionality.

### Fish

Use `ptsolns-cli completion fish > ptsolns-cli.fish` to generate the completion file. At this point you can place the
...
in there with `mv ptsolns-cli.fish ~/.config/fish/completions/`

Remember to open a new shell to test the functionality.

### Powershell

Use `ptsolns-cli completion powershell > ptsolns-cli.ps1` to generate a temporary completion file. At this point you
need to add the content of the generated file to your PowerShell profile file.

1. `Get-Content -Path ptsolns-cli.ps1 | Add-Content -Path $profile` or add it by hand with your favourite text editor.
...
1. `del ptsolns-cli.ps1` to remove the temporary file.
...
If you install the `ptsolns-cli` using [homebrew](https://brew.sh/) package manager the completion should work out of
