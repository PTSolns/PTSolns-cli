...
- `library` - configuration options relating to Arduino libraries.
  - `enable_unsafe_install` - set to `true` to enable the use of the `--git-url` and `--zip-file` flags with
    [`ptsolns-cli lib install`][arduino cli lib install]. These are considered "unsafe" installation methods because
...
- `sketch` - configuration options relating to [Arduino sketches][sketch specification].
  - `always_export_binaries` - set to `true` to make [`ptsolns-cli compile`][ptsolns-cli compile] always save binaries
    to the sketch folder. This is the equivalent of using the [`--export-binaries`][ptsolns-cli compile options] flag.
...
- The `directories.data` default is OS-dependent:
  - on Linux (and other Unix-based OS) is: `{HOME}/.ptsolns15`
  - on Windows is: `{HOME}/AppData/Local/ptsolns15`
  - on MacOS is: `{HOME}/Library/ptsolns15`
...
[`ptsolns-cli config dump`][ptsolns-cli config dump] displays the current configuration values.
...
Setting an additional Boards Manager URL using the [`--additional-urls`][ptsolns-cli global flags] command line flag:

```shell
$ ptsolns-cli core update-index --additional-urls https://downloads.arduino.cc/packages/package_staging_index.json
```
...
[`ptsolns-cli config init`][ptsolns-cli config init] creates a new empty configuration file.

This allows saving the options set by command line flags or environment variables. For example:

```sh
ptsolns-cli config init --additional-urls https://downloads.arduino.cc/packages/package_staging_index.json
```

#### Locations

The default configuration file is named `ptsolns-cli.yaml`. The configuration file is searched in the following
...
```sh
ptsolns-cli config dump --verbose
```
...
[arduino cli lib install]: commands/ptsolns-cli_lib_install.md
[sketch specification]: sketch-specification.md
[ptsolns-cli compile]: commands/ptsolns-cli_compile.md
[ptsolns-cli compile options]: commands/ptsolns-cli_compile.md#options
[ptsolns-cli config dump]: commands/ptsolns-cli_config_dump.md
[arduino cli command reference]: commands/ptsolns-cli.md
[ptsolns-cli global flags]: commands/ptsolns-cli_config.md#options-inherited-from-parent-commands
...
[ptsolns-cli config init]: commands/ptsolns-cli_config_init.md
